var wheellwidth = $(".wheell").width(),
  wheellheight = $(".wheell").height(),
...
var svg3 = d3v3.select(".wheell").append("svg")
  .attr("width", width)
  .attr("height", height)
...
d3v3.csv('aster2.csv', function (error, data) {
  ...
  var outerPath = svg3.append("g").attr("class","out").classed("fan",true).selectAll(".outlineArc")
    .data(pie(data2))
    .enter().append("path")
    .attr("class", (d) => d.data.label)
    .attr("fill", function (d) {
